...
def process_new_source_event(msg):
    ...
    payload = metadata["kanbanizePayload"]

    e_id = payload["card"]["boardName"] + "/" + payload["card"]["taskid"]
    created_at = payload["timestamp"]
    signature = shared.create_unique_id(msg)
    source = payload["card"]["boardName"]

    # [TODO: Parse the msg data to map to the event object below]
    kanbanize_event = {
        "event_type": "issues",  # Event type, eg "push", "pull_reqest", etc
